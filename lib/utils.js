	...
	this.makeRequest = function (methodName, opt, bc) {
		...
		req.on('error', function (err) {
			if (err) {
				...
				log.RunLog('error', methodName, server, null, startTime, msg, err['errno'], "error Msg:[" + headerTostring(err) + "]");
				bc(err, "connectErr");
				return;
			}
		});
	...
	function getRequest(methodName, serverback, filePath, startTime, sendMsg, bc) {
		...
		if (filePath != null && serverback.statusCode < 300) {
			...
			stream.on('error', function (err) {
				...
				log.RunLog('error', methodName, server, null, startTime, sendMsg, err['errno'], "error Msg:[" + headerTostring(err) + "]");
				bc(err, null);
				return;
			});
			...
			serverback.on('end', function () {
				...
				log.RunLog('info', methodName, server, null, startTime, sendMsg, "0", "Status :" + opt['CommonMsg']['Status'] + ", headers:" + headerTostring(serverback.headers));
				bc(null, opt);
				return;
			});
		}
			...
			serverback.on('data', function (data) {
				...
				log.RunLog('info', methodName, server, null, startTime, sendMsg, "0", respMsg);
